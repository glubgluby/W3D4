...
def harrison_ford
  ...
  Movie.joins(:actors).select(:id, :title).where.not(castings: {ord: 1}).where(actors: {name: 'Harrison Ford'})
end
...
def biggest_cast
  ...
  Movie.joins(:actors).select(:id, :title).group(:id).order('COUNT(*) DESC').limit(3)
